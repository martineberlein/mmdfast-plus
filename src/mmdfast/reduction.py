...
class Reducer:
    ...
    def __init__(self, top_n=None, threshold=None, random_state=0):
        ...
        LOOGER.info("Reducer initialized with top_n=%s, threshold=%s, random_state=%d",
    ...
    def fit(self, X, y):
        ...
        if self.top_n is not None:
            ...
            LOOGER.info("Selected top %d features: %s", self.top_n, self.selected_features)
            ...
            LOOGER.info("Selected features with importance >= %.3f: %s", self.threshold, self.selected_features)
        else:
            ...
            LOOGER.info("No top_n or threshold provided. Using mean importance (%.3f). Selected features: %s",
    ...
    def transform(self, X):
        ...
        if self.selected_features is None:
            LOOGER.error("The reducer has not been fitted yet. Please call fit() before transform().")
            raise ValueError("Reducer is not fitted yet.")
        LOOGER.info("Reducing data to the selected features: %s", self.selected_features)
    ...
    def fit_transform(self, X, y):
        ...
        """
        LOOGER.info("Performing fit_transform on the data.")
        self.fit(X, y)
